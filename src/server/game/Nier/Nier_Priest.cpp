...
{
    dpsDistance = VISIBILITY_COMPENSATION;
    followDistance = VISIBILITY_COMPENSATION;
...
bool Nier_Priest::Cure()
{
    if (spell_DispelMagic > 0 || spell_CureDisease > 0)
    {
        if (Group* group = me->GetGroup())
        {
            for (GroupReference* groupRef = group->GetFirstMember(); groupRef != nullptr; groupRef = groupRef->next())
            {
                if (Player* member = groupRef->GetSource())
                {
                    ...
                    Unit::VisibleAuraMap const* visibleAuras = member->GetVisibleAuras();
        ...
        {
            Unit::VisibleAuraMap const* visibleAuras = me->GetVisibleAuras();
            for (Unit::VisibleAuraMap::const_iterator itr = visibleAuras->begin(); itr != visibleAuras->end(); ++itr)
            {
                if (Aura* aura = itr->second->GetBase())
                {
