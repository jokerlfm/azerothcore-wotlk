...
namespace
{
    ...
    bool ParseFile(std::string const& file, bool isOptional, bool isReload)
    {
        ...
        if (in.fail())
        {
            ...
            throw NierConfigException(Acore::StringFormat("Config::LoadFile: Failed open {}file '{}'", isOptional ? "optional " : "", file));
        }
        ...
        while (in.good())
        {
            ...
            if (!in.good() && !in.eof())
            {
                throw NierConfigException(Acore::StringFormat("> Config::LoadFile: Failure to read line number {} in file '{}'", lineNumber, file));
            }
        ...
        if (!count)
        {
            ...
            throw NierConfigException(Acore::StringFormat("Config::LoadFile: Empty file '{}'", file));
        }
