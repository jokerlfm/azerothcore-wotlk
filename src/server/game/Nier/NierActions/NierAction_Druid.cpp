...
bool NierAction_Druid::DPS(Unit* pmTarget, bool pmRushing, float pmDistanceMax, float pmDistanceMin, bool pmHolding, bool pmInstantOnly, bool pmChasing, bool pmForceBack)
{
    if (!me)
    {
        return false;
...
bool NierAction_Druid::AOE(Unit* pmTarget, bool pmRushing, float pmDistanceMax, float pmDistanceMin, bool pmHolding, bool pmInstantOnly, bool pmChasing)
{
    ...
    if (pmChasing)
    {
        if (!movement->Chase(pmTarget, pmDistanceMax, pmDistanceMin, pmHolding, false))
        {
            if (me->GetTarget() == pmTarget->GetGUID())
            {
                ClearTarget();
        ...
        if (spell_EntanglingRoots > 0)
        {
            if (me)
            {
                if (pmTarget)
                {
                    if (Group* myGroup = me->GetGroup())
                    {
                        if (myGroup->GetTargetIconByGuid(pmTarget->GetGUID()) < 0)
                        {
                            if (Creature* targetC = (Creature*)pmTarget)
                            {
                                if (const CreatureTemplate* ci = targetC->GetCreatureTemplate())
                                {
                                    if (const SpellInfo* pS = sSpellMgr->GetSpellInfo(spell_EntanglingRoots))
                                    {
                                        if (!targetC->IsImmunedToSpell(pS))
                                        {
                                            if (me->IsValidAttackTarget(targetC))
                                            {
                                                myGroup->SetTargetIcon(pmRTI, me->GetGUID(), pmTarget->GetGUID());
                                                return true;
                                            }
                                        }
                                    }
                        ...
                        if (myGroup->GetTargetIconByGuid(pmTarget->GetGUID()) < 0)
                        {
                            if (Creature* targetC = (Creature*)pmTarget)
                            {
                                if (const CreatureTemplate* ci = targetC->GetCreatureTemplate())
                                {
                                    ...
                                    {
                                        if (const SpellInfo* pS = sSpellMgr->GetSpellInfo(spell_Hibernate))
                                        {
                                            if (!targetC->IsImmunedToSpell(pS))
                                            {
                                                if (me->IsValidAttackTarget(targetC))
                                ...
                                if (Creature* targetC = (Creature*)rtiTarget)
                                {
                                    if (const CreatureTemplate* ci = targetC->GetCreatureTemplate())
                                    {
                                            ...
                                            if (const SpellInfo* pS = sSpellMgr->GetSpellInfo(spell_Hibernate))
                                            {
                                                if (!targetC->IsImmunedToSpell(pS))
                                                {
                                                    if (me->IsValidAttackTarget(targetC))
                                                    {
                                                        ...
                                                        {
                                                            int duration = 0;
                                                            if (Aura* activeAura = targetC->GetAura(spell_Hibernate))
                                                            {
                                                                duration = activeAura->GetDuration();
                                                            }
                                                            if (duration < 2000)
                                                            {
                                        ...
                                        if (!canAssist)
                                        {
                                                ...
                                                if (const SpellInfo* pS = sSpellMgr->GetSpellInfo(spell_EntanglingRoots))
                                                {
                                                    if (!targetC->IsImmunedToSpell(pS))
                                                    {
                                                        if (me->IsValidAttackTarget(targetC))
                                                        {
                                                            ...
                                                            {
                                                                int duration = 0;
                                                                if (Aura* activeAura = targetC->GetAura(spell_EntanglingRoots))
                                                                {
                                                                    duration = activeAura->GetDuration();
                                                                }
                                                                if (duration < 2000)
                                            ...
                                            if (!canAssist)
                                            {
                                                myGroup->SetTargetIcon(pmRTI, me->GetGUID(), ObjectGuid());
                                            }
                                        }
                                    }
                                }
