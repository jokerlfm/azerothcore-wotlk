...
bool NierAction_Mage::DPS(Unit* pmTarget, bool pmRushing, float pmDistanceMax, float pmDistanceMin, bool pmHolding, bool pmInstantOnly, bool pmChasing, bool pmForceBack)
{
    if (!me)
    {
        return false;
                        ...
                        if (myGroup->GetTargetIconByGuid(pmTarget->GetGUID()) < 0)
                        {
                            if (Creature* targetC = (Creature*)pmTarget)
                            {
                                if (const CreatureTemplate* ci = targetC->GetCreatureTemplate())
                                {
                                    ...
                                    {
                                        if (const SpellInfo* pS = sSpellMgr->GetSpellInfo(spell_Polymorph))
                                        {
                                            ...
                                            if (const SpellInfo* pS = sSpellMgr->GetSpellInfo(spell_Polymorph))
                                            {
                                                if (!targetC->IsImmunedToSpell(pS))
                                                {
                                                    if (me->IsValidAttackTarget(targetC))
                                                    {
                                                        ...
                                                        {
                                                            int duration = 0;
                                                            if (Aura* activeAura = targetC->GetAura(spell_Polymorph))
                                                            {
