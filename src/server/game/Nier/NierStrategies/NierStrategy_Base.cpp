...
void NierStrategy_Base::Update(uint32 pmDiff)
{
            ...
            if (basicStrategyType == BasicStrategyType::BasicStrategyType_Glue)
            {
                ...
                if (Player* leader = ObjectAccessor::FindPlayer(myGroup->GetLeaderGUID()))
                {
                    if (leader->IsAlive())
                    {
                        ...
                        {
                            me->nierAction->movement->ResetMovement();
                            me->InterruptNonMeleeSpells(true);
    ...
    else
    {
        if (relocateDelay >= 0)
        {
            relocateDelay -= pmDiff;
            if (relocateDelay <= 0)
            {
                relocateDelay = urand(sNierConfig->RelocateDelay, sNierConfig->RelocateDelay * 2);
                ObjectGuid masterGuid = ObjectGuid(HighGuid::Player, masterId);
                if (Player* master = ObjectAccessor::FindConnectedPlayer(masterGuid))
                {
                    if (Map* masterMap = master->GetMap())
                    {
                        if (!masterMap->Instanceable())
                        {
                            ...
                            master->GetNearPoint(master, nearX, nearY, nearZ, 0.0f, nearDistance, nearAngle);
                            me->ClearInCombat();
        ...
        else
        {
            ...
            {
                reviveDelay = urand(sNierConfig->ReviveDelay, sNierConfig->ReviveDelay * 2);
            }
        }
    }
}
...
bool NierStrategy_Base::TryDPS(bool pmForceInstantOnly, bool pmChasing)
{
    if (!me)
    {
        return false;
        ...
        if (Player* leader = ObjectAccessor::FindConnectedPlayer(myGroup->GetLeaderGUID()))
        {
            if (Unit* leaderTarget = leader->GetSelectedUnit())
            {
                if (leaderTarget->IsInCombat())
                {
                    ...
                    {
                        if (leader->IsWithinMeleeRange(leaderTarget))
                        {
...
bool NierStrategy_Base::Wander()
{
    ...
    if (wanderRate < 50)
    {
        ...
        me->nierAction->movement->Point(dest);
    }
...
bool NierStrategy_Magisters_Terrace::TryDPS(bool pmForceInstantOnly, bool pmChasing)
{
    if (!me)
    {
        return false;
    ...
    else if (arak)
    {
            ...
            if (Spell* pounding = boss->GetCurrentSpell(CurrentSpellTypes::CURRENT_GENERIC_SPELL))
            {
                if (pounding->m_spellInfo->Id == 53472 || pounding->m_spellInfo->Id == 59433)
                {
                    ...
                    if (boss->isInFront(me, NIER_NORMAL_DISTANCE))
                    {
                        ...
                        actionLimit = 2000;
                        actionType = ActionType::ActionType_Move;
                        me->nierAction->movement->Point(dodgePos, actionLimit);
                        return;
                    }
                }
            }
        ...
        else
        {
            ...
            if (centerDistance > NIER_FAR_DISTANCE)
            {
                ...
                actionLimit = 2000;
                actionType = ActionType::ActionType_Move;
                me->nierAction->movement->Point(inPos, actionLimit);
                return;
            }
        }
    }
...
bool NierStrategy_Azjol_Nerub::DoTank(Unit* pmTarget)
{
    ...
    if (pmTarget->GetEntry() == 29120 || pmTarget->GetEntry() == 31610)
    {
        ...
        //                    actionLimit = 2000;
        //                    actionType = ActionType::ActionType_Move;
        //                    me->nierAction->movement->Point(tankPos, actionLimit);
        //                    return true;
        //                }
        //            }
        //        }
