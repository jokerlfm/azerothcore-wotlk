...
namespace
{
    ...
    bool ParseFile(std::string const& file, bool isOptional, bool isReload)
    {
        ...
        if (in.fail())
        {
            ...
            throw MingConfigException(Acore::StringFormatFmt("Config::LoadFile: Failed open {}file '{}'", isOptional ? "optional " : "", file));
        }
        ...
        while (in.good())
        {
            ...
            if (!in.good() && !in.eof())
            {
                throw MingConfigException(Acore::StringFormatFmt("> Config::LoadFile: Failure to read line number {} in file '{}'", lineNumber, file));
            }
        ...
        if (!count)
        {
            ...
            throw MingConfigException(Acore::StringFormatFmt("Config::LoadFile: Empty file '{}'", file));
        }
...
bool MingConfig::LoadInitial(std::string const& file, bool isReload /*= false*/)
{
...
bool MingConfig::LoadAdditionalFile(std::string file, bool isOptional /*= false*/, bool isReload /*= false*/)
{
    std::lock_guard<std::mutex> lock(_configLock);
...
template<class T>
T MingConfig::GetValueDefault(std::string const& name, T const& def, bool showLogs /*= true*/) const
{
...
template<>
std::string MingConfig::GetValueDefault<std::string>(std::string const& name, std::string const& def, bool showLogs /*= true*/) const
{
    auto const& itr = _configOptions.find(name);
    if (itr == _configOptions.end())
    {
...
template<class T>
T MingConfig::GetOption(std::string const& name, T const& def, bool showLogs /*= true*/) const
{
    return GetValueDefault<T>(name, def, showLogs);
}

template<>
bool MingConfig::GetOption<bool>(std::string const& name, bool const& def, bool showLogs /*= true*/) const
{
...
std::string const MingConfig::GetConfigPath()
{
    std::lock_guard<std::mutex> lock(_configLock);
...
void MingConfig::Configure(std::string const& initFileName, std::vector<std::string> args, std::string_view modulesConfigList /*= {}*/)
{
...
bool MingConfig::LoadModulesConfigs(bool isReload /*= false*/, bool isNeedPrintInfo /*= true*/)
{
    ...
    for (auto const& distFileName : _additonalFiles)
    {
        ...
        {
            LOG_FATAL("server.loading", "> MingConfig::LoadModulesConfigs: Not found original config '{}'. Stop loading", distFileName);
            ABORT();
        }
...
std::string MingConfig::GetStringDefault(std::string const& name, const std::string& def, bool showLogs /*= true*/)
{
    return GetOption<std::string>(name, def, showLogs);
}

// @deprecated DO NOT USE - use GetOption<bool> instead.
bool MingConfig::GetBoolDefault(std::string const& name, bool def, bool showLogs /*= true*/)
{
    return GetOption<bool>(name, def, showLogs);
}

// @deprecated DO NOT USE - use GetOption<int32> instead.
int MingConfig::GetIntDefault(std::string const& name, int def, bool showLogs /*= true*/)
{
    return GetOption<int32>(name, def, showLogs);
}

// @deprecated DO NOT USE - use GetOption<float> instead.
float MingConfig::GetFloatDefault(std::string const& name, float def, bool showLogs /*= true*/)
{
...
{
    std::string configFile = GetConfigPath() + std::string(MING_CONFIG_FILE_NAME);
    std::vector<std::string> args;
    std::string_view moduleList;
    Configure(configFile, args, moduleList);
    if (!LoadAppConfigs())
    {
        sLog->outMessage(MING_MARK, LogLevel::LOG_LEVEL_ERROR, "Error in ming config file : {}.", MING_CONFIG_FILE_NAME);
        return false;
    }
    ...
    if (Enable == 0)
    {
        sLog->outMessage(MING_MARK, LogLevel::LOG_LEVEL_DEBUG, "ming is disabled.");
        return false;
    }
    sLog->outMessage(MING_MARK, LogLevel::LOG_LEVEL_DEBUG, "ming started.");
    return true;
}

#define TEMPLATE_CONFIG_OPTION(__typename) \
    template __typename MingConfig::GetOption<__typename>(std::string const& name, __typename const& def, bool showLogs /*= true*/) const;
