...
namespace
{
    ...
    bool ParseFile(std::string const& file, bool isOptional, bool isReload)
    {
        ...
        if (in.fail())
        {
            ...
            throw LingerConfigException(Acore::StringFormatFmt("Config::LoadFile: Failed open {}file '{}'", isOptional ? "optional " : "", file));
        }
        ...
        while (in.good())
        {
            ...
            if (!in.good() && !in.eof())
            {
                throw LingerConfigException(Acore::StringFormatFmt("> Config::LoadFile: Failure to read line number {} in file '{}'", lineNumber, file));
            }
        ...
        if (!count)
        {
            ...
            throw LingerConfigException(Acore::StringFormatFmt("Config::LoadFile: Empty file '{}'", file));
        }
...
bool LingerConfig::LoadInitial(std::string const& file, bool isReload /*= false*/)
{
...
bool LingerConfig::LoadAdditionalFile(std::string file, bool isOptional /*= false*/, bool isReload /*= false*/)
{
    std::lock_guard<std::mutex> lock(_configLock);
    return LoadFile(file, isOptional, isReload);
}

LingerConfig* LingerConfig::instance()
{
...
template<class T>
T LingerConfig::GetValueDefault(std::string const& name, T const& def, bool showLogs /*= true*/) const
{
...
template<>
std::string LingerConfig::GetValueDefault<std::string>(std::string const& name, std::string const& def, bool showLogs /*= true*/) const
{
    auto const& itr = _configOptions.find(name);
    if (itr == _configOptions.end())
    {
...
template<class T>
T LingerConfig::GetOption(std::string const& name, T const& def, bool showLogs /*= true*/) const
{
    return GetValueDefault<T>(name, def, showLogs);
}

template<>
bool LingerConfig::GetOption<bool>(std::string const& name, bool const& def, bool showLogs /*= true*/) const
{
...
std::vector<std::string> LingerConfig::GetKeysByString(std::string const& name)
{
    std::lock_guard<std::mutex> lock(_configLock);
...
std::string const LingerConfig::GetFilename()
{
    std::lock_guard<std::mutex> lock(_configLock);
    return _usingDistConfig ? _filename + ".dist" : _filename;
}

std::vector<std::string> const& LingerConfig::GetArguments() const
{
    return _args;
}

std::string const LingerConfig::GetConfigPath()
{
    std::lock_guard<std::mutex> lock(_configLock);
...
void LingerConfig::Configure(std::string const& initFileName, std::vector<std::string> args, std::string_view modulesConfigList /*= {}*/)
{
...
bool LingerConfig::LoadAppConfigs(bool isReload /*= false*/)
{
...
bool LingerConfig::LoadModulesConfigs(bool isReload /*= false*/, bool isNeedPrintInfo /*= true*/)
{
    ...
    for (auto const& distFileName : _additonalFiles)
    {
        ...
        {
            LOG_FATAL("server.loading", "> LingerConfig::LoadModulesConfigs: Not found original config '{}'. Stop loading", distFileName);
            ABORT();
        }
...
std::string LingerConfig::GetStringDefault(std::string const& name, const std::string& def, bool showLogs /*= true*/)
{
    return GetOption<std::string>(name, def, showLogs);
}

// @deprecated DO NOT USE - use GetOption<bool> instead.
bool LingerConfig::GetBoolDefault(std::string const& name, bool def, bool showLogs /*= true*/)
{
    return GetOption<bool>(name, def, showLogs);
}

// @deprecated DO NOT USE - use GetOption<int32> instead.
int LingerConfig::GetIntDefault(std::string const& name, int def, bool showLogs /*= true*/)
{
    return GetOption<int32>(name, def, showLogs);
}

// @deprecated DO NOT USE - use GetOption<float> instead.
float LingerConfig::GetFloatDefault(std::string const& name, float def, bool showLogs /*= true*/)
{
...
{
    std::string configFile = GetConfigPath() + std::string(LINGER_CONFIG_FILE_NAME);
    std::vector<std::string> args;
    std::string_view moduleList;
    Configure(configFile, args, moduleList);
    if (!LoadAppConfigs())
    {
        sLog->outMessage(LINGER_MARK, LogLevel::LOG_LEVEL_ERROR, "Error in linger config file : %s.", LINGER_CONFIG_FILE_NAME);
        return false;
    }
    ...
    if (Enable == 0)
    {
        sLog->outMessage(LINGER_MARK, LogLevel::LOG_LEVEL_INFO, "linger is disabled.");
        return false;
    }
    sLog->outMessage(LINGER_MARK, LogLevel::LOG_LEVEL_INFO, "linger started.");
    return true;
}

#define TEMPLATE_CONFIG_OPTION(__typename) \
    template __typename LingerConfig::GetOption<__typename>(std::string const& name, __typename const& def, bool showLogs /*= true*/) const;
